...
50?this.timer+2:0;var a=200+Math.abs(this.timer-25)*2;this.level==LEVEL_LAST&&drawCenterImage("img/5618177568_997688dbaa_b.jpg");drawCenterImage("img/freebee.png");switch(this.level){case -1:drawMessage("G A M E  O V E R","red",130);drawMessage(this.score,"yellow",200);drawMessage("right click to try again","rgb("+a+","+a+","+a+")",-200,20);mouseDown(2)&&this.Restart();break;case 0:drawMessage("tap or click to start","rgb("+a+","+a+","+a+")",-200);drawMessage("help the bees escape! they'll go for sugar",
"#ccc",122,13);drawMessage("the faster you are, the more points you get","#ccc",142,13);drawMessage("race the clock through five challenging levels","#ccc",162,13);drawMessage("a #LD21 game jam entry","#dfd",200,16);mouseDown(0)&&this.Restart();break;case LEVEL_LAST:drawMessage("right click to play again","rgb("+a+","+a+","+a+")",175,13);drawMessage("you saved uzzz, thanks!","#cf0",135);drawMessage(this.score,"yellow",200);mouseDown(2)&&this.Restart();break;default:if(drawMessage("LEVEL "+this.level+
" complete!","#fec",120),drawMessage(this.score,"yellow",200),drawMessage("RIGHT click to continue","rgb("+a+","+a+","+a+")",170,18),mouseDown(2))this.pause=!1,this.Start()}};this.Restart=function(){this.level=0;this.pause=!1;this.score=0;this.Start()};this.Start=function(){this.timer=(new Date).getTime();this.blocks=[];this.bees=[];switch(this.level){case 0:this.createExits(4);this.createBees(3);this.play=new this.Player;break;case 1:this.createBlock(1.5,0,0);this.createExits(3);this.createBees(4);

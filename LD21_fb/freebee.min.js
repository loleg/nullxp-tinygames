...
0.7641856074333191;a.miterLimit=4;a.globalAlpha=0.6995884776115417;a.beginPath();a.moveTo(1.7036023,16.912675);a.lineTo(21.49003,1.1253211);a.lineTo(21.49003,28.861866);a.lineTo(1.7036023,35.401708);a.closePath();a.fill();a.stroke();a.restore();a.restore();a.restore()};var LEVEL_LAST=4;
...
0;b<c;b++)a.save(),a.translate(20+b*25,20),a.scale(-0.3,0.3),a.globalAlpha=0.8,drawBee(a,!0),a.restore()};this.Exit=function(c,b){this.posH=c;this.posV=b;this.s=30;this.capture=14;this.x=this.posH?this.posV?6:a.canvas.width:centerX;this.y=!this.posH?this.posV?6:a.canvas.height:centerY;this.draw=function(){a.save();a.fillStyle="#000000";a.translate(this.x,this.y);a.beginPath();a.arc(0,0,this.s,0,6.2832,!0);a.fill();a.restore()};this.checkBee=function(a){var c=Math.abs(a.y-this.y);if(Math.abs(a.x-this.x)<
this.capture&&c<this.capture)return a.active=!1,!0;return!1};this.checkAllBees=function(){for(var a in game.bees)this.checkBee(game.bees[a])};this.update=function(){this.draw();this.checkAllBees()}};this.createBees=function(c){this.total=c;for(var b=0;b<c;b++){var d=new this.Bee;d.y=(0.5+b)*a.canvas.height/c;d.moveToRandom();this.bees.push(d)}};this.createBlock=function(a,b,d){var e=ww=20,f=new this.Block;d?(ww=a*centerX,f.x-=ww/2,f.y=-(e/2)+centerY+b*centerY*2):(e=a*centerY,f.x=-(ww/2)+centerX+b*
centerX*2,f.y-=e/2);f.w=ww;f.h=e;this.blocks.push(f)};this.createExits=function(a){this.exits=[];var b=new this.Exit(1,0);this.exits.push(b);a!=1&&(b=new this.Exit(1,1),this.exits.push(b),a!=2&&(b=new this.Exit(0,1),this.exits.push(b),a!=3&&(b=new this.Exit(0,0),this.exits.push(b))))};this.Block=function(){this.h=this.w=0;this.x=centerX;this.y=centerY;this.draw=function(){a.save();a.fillStyle="#cb0";a.translate(this.x,this.y);a.fillRect(0,0,this.w,this.h);a.restore()}};this.Player=function(){this.x=
this.y=0;this.s=20;this.draw=function(){a.save();a.translate(this.x-20,this.y-20);drawSugar(a);a.restore()};this.update=function(){this.x=mouseX;this.y=mouseY;this.draw()}};this.Action=function(){var a=30-Math.floor(((new Date).getTime()-this.timer)/1E3),b;for(b in this.exits)this.exits[b].update();for(var d in this.blocks)this.blocks[d].draw();b=[];for(d in this.bees)this.bees[d].active?(mouseDown(0)&&this.bees[d].pullTo(mouseX,mouseY),this.bees[d].update(),b.push(this.bees[d])):this.score+=a;this.bees=
b;mouseDown(0)&&this.play.update();drawMessage(this.score,"yellow",200);this.drawCounter(this.total-this.bees.length);if(this.bees.length==0)this.level++,this.pause=!0;drawMessage(a,a<10?"red":null,-230);if(a<1)this.level=-1,this.pause=!0};this.Notice=function(){this.timer=this.timer<50?this.timer+2:0;var a=200+Math.abs(this.timer-25)*2;this.level==LEVEL_LAST&&drawCenterImage("img/5618177568_997688dbaa_b.jpg");drawCenterImage("img/freebee.png");switch(this.level){case -1:drawMessage("G A M E  O V E R",
"red",130);drawMessage(this.score,"yellow",200);drawMessage("right click to try again","rgb("+a+","+a+","+a+")",170,18);mouseDown(2)&&this.Restart();break;case 0:drawMessage("tap or click to start","rgb("+a+","+a+","+a+")",130);drawMessage("help the bees escape with sugar!","#ccc",170,13);drawMessage("a #LD21 game jam entry","#dfd",200,16);mouseDown(0)&&this.Restart();break;case LEVEL_LAST:drawMessage("right click to play again","rgb("+a+","+a+","+a+")",170,13);drawMessage("thanks! you saved uzzz, hero :)",
"#cfc",130);drawMessage(this.score,"yellow",200);mouseDown(2)&&this.Restart();break;default:if(drawMessage("LEVEL "+this.level+" complete!","#fec",120),drawMessage(this.score,"yellow",200),drawMessage("RIGHT click to continue","rgb("+a+","+a+","+a+")",170,18),mouseDown(2))this.pause=!1,this.Start()}};this.Restart=function(){this.pause=!1;this.level=this.score=0;this.Start()};this.Start=function(){this.timer=(new Date).getTime();this.blocks=[];this.bees=[];switch(this.level){case 0:this.createExits(4);
this.createBees(3);this.play=new this.Player;break;case 1:this.createBlock(1,-0.2,1);this.createBlock(1,0.2,1);this.createExits(2);this.createBees(5);this.play=new this.Player;break;case 2:this.createBlock(0.7,-0.3,0);this.createBlock(0.7,0.3,0);this.createBlock(0.7,-0.3,1);this.createBlock(0.7,0.3,1);this.createExits(4);this.createBees(7);this.play=new this.Player;break;case 3:this.createBlock(1,-0.2,0),this.createBlock(1,0.2,0),this.createExits(2),this.createBees(7),this.play=new this.Player}};
this.Frame=function(){this.pause?this.Notice():this.Action()}};var FPS=30,canvas=null,Context2d=null,game=null,centerX=centerY=0,MouseArray=Array(3),mouseX=0,mouseY=0;function initCanvas(a){a.onmousedown=function(a){MouseArray[a.button]=!0;return!1};a.onmouseup=function(a){MouseArray[a.button]=!1};a.onmousemove=function(a){mouseX=a.clientX-canvas.offsetLeft+document.body.scrollLeft;mouseY=a.clientY-canvas.offsetTop+document.body.scrollTop}}function mouseDown(a){return MouseArray[a]}
...
function drawMessage(a,c,b,d){c=!c?"#fff":c;b=!b?0:b;d=!d?24:d;Context2d.save();Context2d.font="bold "+d+"pt sans-serif";Context2d.textBaseline="top";Context2d.textAlign="center";Context2d.fillStyle=c;Context2d.shadowColor="#333";Context2d.shadowOffsetX=3;Context2d.shadowOffsetY=3;Context2d.shadowBlur=5;Context2d.fillText(a,centerX,centerY+b);Context2d.restore()}
function drawCenterImage(a){Context2d.save();var c=new Image;c.src=a;Context2d.drawImage(c,centerX-c.width/2,centerY-c.height/2);Context2d.restore()};
